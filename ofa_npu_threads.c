...
/*! \struct rbuff_t
 *  \brief  ring buffer with configurable size
 *
 *  A ring buffer of void *s, that'll hold addresses of (pointers to) messages.
 *  The size is set during initialization. For a single producer-consumer use,
 *  the buffer does not need synchronization between post and get accesses to
 *  the buffer. The accesses are through two different pointers for each access
 *  (post_ptr and get_ptr)
 */
...
ofa_cerrno
rbuff_init (rbuff_t *buff, size_t size) {
    buff->head = (void *) malloc(size * sizeof(void*));
    memset(buff->head, 0, size*sizeof(void*));
    buff->size = size;
    buff->post_index = 0;
    buff->get_index = 0;
    return OFA_EOK;
...
ofa_cerrno
rbuff_post (rbuff_t *buff, void *msg) {
    if (buff->head[buff->post_index] != NULL) {
        return OFA_EBUSY;
    }
    buff->head[buff->post_index] = msg;
    buff->post_index = (buff->post_index+1) % buff->size;
    return OFA_EOK;
}

ofa_cerrno
rbuff_get (rbuff_t *buff, void **msg) {
    (*msg) = NULL;
    if (buff->head[buff->get_index] != NULL) {
        (*msg) = buff->head[buff->get_index];
        buff->head[buff->get_index] = NULL;
        buff->get_index = (buff->get_index+1) % buff->size;
    }
    return OFA_EOK;
}
...
typedef struct _ofa_npu_thread_t {
    ...
    pthread_cond_t  cond;
    rbuff_t         msg_buff;   /* ring buffer to hold pointer to messages */
    queue_t         msg_queue;
...
ofa_npu_thread_main (void * args)
{
    ...
    while (1) {
        void *msg = NULL;
        //rbuff_get(&npu_thread->msg_buff, &msg);
        if (queue_get(&npu_thread->msg_queue, &msg) == OFA_EOK) {
            OFA_LTRACE(npu_thread->id, "\nthread (%d) %s", npu_thread->id, (char *)msg);
            ofa_npu_thread_process_msg(msg);
            count++;
        } else {
